...
function getRoleMeta(role?: string) {
  if (role === "raja") return { icon: "👑", label: "Raja", tone: "text-[#ffd766]" };
  if (role === "mantri") return { icon: "📜", label: "Mantri", tone: "text-[#d8b4fe]" };
  if (role === "chor") return { icon: "🕵️", label: "Chor", tone: "text-[#ff8a65]" };
  if (role === "sipahi") return { icon: "🛡️", label: "Sipahi", tone: "text-[#7dd3fc]" };
...
export function ResultCard({ result, players, myRole, currentRound, totalRounds, gameFinished, onNextRound, onPlayAgain, showNextRound }: ResultCardProps) {
    ...
    <div className="relative mx-auto max-w-6xl overflow-hidden rounded-[28px] border border-[#ffd766]/20 bg-[linear-gradient(145deg,rgba(255,244,215,0.14)_0%,rgba(55,24,15,0.84)_46%,rgba(20,9,7,0.94)_100%)] px-3 py-4 text-white shadow-[0_24px_70px_rgba(0,0,0,0.38)] backdrop-blur-2xl sm:px-4 sm:py-5">
            ...
            {players.map((player, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="relative z-10 mx-auto flex h-9 w-9 items-center justify-center rounded-xl bg-white/10 text-2xl shadow-[inset_0_1px_0_rgba(255,255,255,0.18)]">
                    {role.icon}
                  </div>
      ...
      <div className="relative z-10 mt-4 flex flex-col items-center justify-center gap-2 sm:flex-row">
        ...
        <button
          type="button"
          disabled
          className="w-full cursor-not-allowed rounded-full border border-white/[0.08] bg-white/[0.035] px-6 py-3 font-ui text-sm font-black uppercase tracking-[0.18em] text-[#ffe8b8]/45 sm:w-auto"
        >
          View Summary
        </button>
      </div>
